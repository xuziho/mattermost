...
func (s *MmctlE2ETestSuite) TestDeleteTeamsCmdF() {
	...
	s.Run("Permission denied error for system admin when deleting a valid team", func() {
		...
		s.Len(printer.GetLines(), 0)
		s.Len(printer.GetErrorLines(), 1)
		s.Equal("Unable to delete team '"+s.th.BasicTeam.Name+"' error: Permanent team deletion feature is not enabled. ServiceSettings.EnableAPITeamDeletion must be set to true to use this command.", printer.GetErrorLines()[0])
