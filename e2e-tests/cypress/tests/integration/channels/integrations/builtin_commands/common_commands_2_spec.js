    ...
    it('MM-T710 /mute error message', () => {
        ...
        cy.getLastPostId().then((postId) => {
            ...
            cy.get(`#postMessageText_${postId}`).
                ...
                // * Channel handle links to AgentCompanyOS channel naming guidance.
                contains('a', 'channel handle').then((link) => {
                    const href = link.prop('href');
                    cy.request(href).its('allRequestResponses').then((response) => {
                        cy.wrap(response[0]['Request URL']).should('equal', 'https://agentcompanyos.local/docs/channels#naming-a-channel');
                    });
                });
        });
    });
